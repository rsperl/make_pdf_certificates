...
if [[ ! -f "$template_pdf" ]]; then
  usage "template not found: $template_pdf"
  exit 1
fi
template_pdf="$(realpath "$template_pdf")"

if [[ ! -f "$template_create" ]]; then
  usage "template create json file not found: $template_create"
  exit 1
fi
template_create="$(realpath "$template_create")"

if [[ ! -f "$names_file" ]]; then
  usage "names file not found: $names_file"
  exit 1
fi
names_file="$(realpath "$names_file")"
...
echo "Template:             $template_pdf"
echo "Template Create JSON: $template_create"
...
# loop through names and add to JSON, output to $create_file
